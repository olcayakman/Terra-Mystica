...
public class Main {
    ...
    public static void main(String[] args) throws MalformedURLException {
       // GameUI ui = new GameUI();
        ...
        ActionHandler.getInstance().setCurrentPlayer(players.get(0));
        players.get(0).getFaction().getAsset().performIncrementalTransaction(new Asset(99, 99, 99, 0));
        players.get(0).getFaction().incrementPower(10);

        // Play the game
        for (int i = 0; i < 4; i++) {
            ActionHandler.getInstance().setTerrainXPosition(i);
            ActionHandler.getInstance().setTerrainYPosition(0);
            ActionHandler.getInstance().setActionID(0);
            ActionHandler.getInstance().executeAction();
        }  
        for (int i = 0; i < 4; i++) {
            ActionHandler.getInstance().setTerrainXPosition(i);
            ActionHandler.getInstance().setTerrainYPosition(0);
            ActionHandler.getInstance().setActionID(3);
            ActionHandler.getInstance().executeAction();
        }  
        ActionHandler.getInstance().executeAction();
        ActionHandler.getInstance().executeAction();
